...
class ServerTestFixture : public testing::Test {
 ...
 protected:
  void SetUp() override {
    ...
    server_thread_ =
        std::make_unique<std::jthread>([this]() { app_->serve(); });
  ...
  [[nodiscard]] static std::vector<std::byte> send_and_receive(
    ...
    const auto _endpoints =
        _resolver.resolve("127.0.0.1", std::to_string(1337));
    ...
    boost::asio::read(_socket,
                      boost::asio::buffer(_response.data(), _response.size()));
...
TEST_F(ServerTestFixture, HandlesSingleValidRequest) {
  const auto _buffer =
      request_insert_builder(1, ttl_types::seconds, 32, "consumer1/resource1");
...
TEST_F(ServerTestFixture, HandlesMultipleSingleRequests) {
  const auto _buffer =
      request_insert_builder(1, ttl_types::seconds, 32, "consumer2/resource2");
...
TEST_F(ServerTestFixture, HandlesTwoConcatenatedRequests) {
  const auto buffer1 =
      request_insert_builder(1, ttl_types::seconds, 64, "consumer3/resource3");
  const auto buffer2 =
      request_insert_builder(1, ttl_types::seconds, 64, "consumer4/resource4");
...
TEST_F(ServerTestFixture, HandleSetSimpleValue) {
  ...
  const std::vector _value = {std::byte{0xBE}, std::byte{0xEF}, std::byte{0xCA},
                              std::byte{0xFE}};

  const auto _buffer =
      request_set_builder(_value, ttl_types::seconds, 10, _key);
...
TEST_F(ServerTestFixture, HandleGetReturnsCorrectValue) {
  ...
  const std::vector<std::byte> _value = {std::byte{0xBA}, std::byte{0xAD},
                                         std::byte{0xF0}, std::byte{0x0D}};

  // 1. SET
  const auto _set_buffer =
      request_set_builder(_value, ttl_types::seconds, 3, _key);
  ...
  const auto _get_response = send_and_receive(
      _get_buffer, 1 + 1 + sizeof(value_type) * 2 + _value.size());

  ASSERT_EQ(_get_response.size(),
            1 + 1 + sizeof(value_type) * 2 + _value.size());

  // Verifica status
  ASSERT_EQ(static_cast<uint8_t>(_get_response[0]), 1);

  // ttl_type
  ASSERT_EQ(static_cast<uint8_t>(_get_response[1]),
            static_cast<uint8_t>(ttl_types::seconds));
  ...
  std::memcpy(&_value_size, _get_response.data() + 2 + sizeof(_ttl),
              sizeof(_value_size));
  ASSERT_EQ(_value_size, _value.size());

  // value
  const auto* _value_ptr =
      _get_response.data() + 2 + sizeof(_ttl) + sizeof(_value_size);
...
TEST_F(ServerTestFixture, TTLExpiration) {
  const auto _insert_buffer = request_insert_builder(
      32, ttl_types::milliseconds, 28, "consumer3/expire");
  ...
  auto _success_query_response =
      send_and_receive(_query_buffer, 2 + (sizeof(value_type) * 2));
  ...
  // [o] [o][o] [x] [o][o]
  const auto _success_response_ttl_type =
      static_cast<uint8_t>(_success_query_response[sizeof(value_type) + 1]);
  ASSERT_EQ(_success_response_ttl_type,
            static_cast<uint8_t>(ttl_types::milliseconds));

  value_type _success_response_ttl;

  // [o] [o][o] [o] [x][x]
  std::memcpy(&_success_response_ttl,
              _success_query_response.data() + sizeof(value_type) + 2,
              sizeof(value_type));
...
TEST_F(ServerTestFixture, SeparateStocksForDifferentIDs) {
  const auto _buffer_a =
      request_insert_builder(3, ttl_types::seconds, 7, "consumerA/resourceA");
  const auto _buffer_b =
      request_insert_builder(5, ttl_types::seconds, 7, "consumerB/resourceB");
...
TEST_F(ServerTestFixture, QueryAfterInsertReturnsCorrectQuota) {
  const auto _insert_buffer = request_insert_builder(
      10, ttl_types::seconds, 16, "consumer_query2/resource_query2");

  auto _ignored = send_and_receive(_insert_buffer);
  boost::ignore_unused(_ignored);

  const auto _query_buffer =
      request_query_builder("consumer_query2/resource_query2");
  auto _response =
      send_and_receive(_query_buffer, 2 + (sizeof(value_type) * 2));
  ...
  std::memcpy(&_quota_remaining, _response.data() + 1,
              sizeof(_quota_remaining));
  ASSERT_EQ(_quota_remaining, 10);
}

TEST_F(ServerTestFixture, QueryExpiredReturnsZeroQuota) {
  const auto _insert_buffer = request_insert_builder(
      0, ttl_types::seconds, 1, "consumer_query3/resource_query3");
  ...
  const auto _query_buffer =
      request_query_builder("consumer_query3/resource_query3");
...
TEST_F(ServerTestFixture, UpdateIncreaseQuota) {
  const auto _insert_buffer = request_insert_builder(0, ttl_types::seconds, 77,
                                                     "consumer/increase_quota");

  auto _ignored = send_and_receive(_insert_buffer);
  boost::ignore_unused(_ignored);

  const auto _update_buffer =
      request_update_builder(attribute_types::quota, change_types::increase, 10,
                             "consumer/increase_quota");
...
TEST_F(ServerTestFixture, UpdateDecreaseQuota) {
  const auto _insert = request_insert_builder(10, ttl_types::seconds, 32,
                                              "consumer/decrease_quota");

  auto _ignored = send_and_receive(_insert);
  boost::ignore_unused(_ignored);

  const auto _update =
      request_update_builder(attribute_types::quota, change_types::decrease, 4,
                             "consumer/decrease_quota");
  ...
  const auto _query_response =
      send_and_receive(_query, 2 + (sizeof(value_type) * 2));
...
TEST_F(ServerTestFixture, UpdatePatchQuota) {
  const auto _insert = request_insert_builder(10, ttl_types::milliseconds, 64,
                                              "consumer/patch_quota");

  auto _ignored = send_and_receive(_insert);
  boost::ignore_unused(_ignored);

  const auto _update = request_update_builder(
      attribute_types::quota, change_types::patch, 4, "consumer/patch_quota");
  auto _update_response = send_and_receive(_update);
  ASSERT_EQ(_update_response.size(), 1);
...
TEST_F(ServerTestFixture, UpdatePatchTTL) {
  const auto _insert = request_insert_builder(10, ttl_types::milliseconds, 32,
                                              "consumer/patch_ttl");
  ...
  const auto _update = request_update_builder(
      attribute_types::ttl, change_types::patch, 64, "consumer/patch_ttl");
  auto _update_response = send_and_receive(_update);
  ASSERT_EQ(_update_response.size(), 1);
  ASSERT_EQ(static_cast<uint8_t>(_update_response[0]), 1);
  ...
  const auto _ttl_type =
      static_cast<uint8_t>(_query_response[1 + sizeof(_quota)]);
...
TEST_F(ServerTestFixture, UpdateNonExistentKeyReturnsError) {
  const auto _update =
      request_update_builder(attribute_types::quota, change_types::patch, 100,
...
TEST_F(ServerTestFixture, UpdateDecreaseQuotaBeyondZero) {
  const auto _insert = request_insert_builder(
      0, ttl_types::seconds, 32, "consumer_beyond/resource_beyond");

  auto _ignored = send_and_receive(_insert);
  boost::ignore_unused(_ignored);

  const auto _update =
      request_update_builder(attribute_types::quota, change_types::decrease, 10,
                             "consumer_beyond/resource_beyond");
  auto _update_response = send_and_receive(_update);
  ASSERT_EQ(_update_response.size(), 1);
  ASSERT_EQ(static_cast<uint8_t>(_update_response[0]), 0);
}
...
TEST_F(ServerTestFixture, UpdatePatchTTLSeconds) {
  const auto _insert = request_insert_builder(0, ttl_types::seconds, 5,
                                              "consumer_sec/resource_sec");

  auto _ignored = send_and_receive(_insert);
  boost::ignore_unused(_ignored);

  const auto _update =
      request_update_builder(attribute_types::ttl, change_types::patch, 10,
                             "consumer_sec/resource_sec");
...
TEST_F(ServerTestFixture, UpdateIncreaseTTL) {
  const auto _insert = request_insert_builder(0, ttl_types::milliseconds, 32,
                                              "consumer_inc/resource_inc");
  auto _ignored = send_and_receive(_insert);
  boost::ignore_unused(_ignored);

  const auto _update =
      request_update_builder(attribute_types::ttl, change_types::increase, 18,
                             "consumer_inc/resource_inc");
  auto _update_response = send_and_receive(_update);
...
TEST_F(ServerTestFixture, UpdateDecreaseTTL) {
  const auto _insert = request_insert_builder(0, ttl_types::milliseconds, 32,
                                              "consumer_dec/resource_dec");
  auto _ignored = send_and_receive(_insert);
  boost::ignore_unused(_ignored);

  const auto _update =
      request_update_builder(attribute_types::ttl, change_types::decrease, 12,
                             "consumer_dec/resource_dec");
  auto _update_response = send_and_receive(_update);
...
TEST_F(ServerTestFixture, PurgeExistingEntry) {
  const auto _insert = request_insert_builder(1, ttl_types::seconds, 32,
                                              "consumer_purge/resource_purge");
...
TEST_F(ServerTestFixture, PurgeNonExistentEntryReturnsError) {
  const auto _purge =
      request_purge_builder("nonexistent_consumer/nonexistent_resource");
  auto _purge_response = send_and_receive(_purge);
