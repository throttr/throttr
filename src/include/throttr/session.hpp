...
class session {
 public:
  ...
  ~session() {
    // LCOV_EXCL_START
#ifndef NDEBUG
    fmt::println("{:%Y-%m-%d %H:%M:%S} SESSION CLOSED ip={} port={}",
                 std::chrono::system_clock::now(), ip_, port_);
#endif
    // LCOV_EXCL_STOP
  }
  ...
  void start() {
    // LCOV_EXCL_START
#ifndef NDEBUG
    fmt::println("{:%Y-%m-%d %H:%M:%S} SESSION ESTABLISHED ip={} port={}",
                 std::chrono::system_clock::now(), ip_, port_);
#endif
    // LCOV_EXCL_STOP
    do_read();
  ...
  void compact_buffer_if_needed() {
    if (buffer_start_ ==
        buffer_end_) {  // LCOV_EXCL_LINE note: Partially tested.
      buffer_start_ = 0;
      buffer_end_ = 0;
    } else if (buffer_start_ >
               max_length_ / 2) {  // LCOV_EXCL_LINE note: Partially tested.
      compact_buffer();
    }
  }
  ...
  void compact_buffer() {
    ...
    std::memmove(buffer_.data(), buffer_.data() + buffer_start_,
 ...
 private:
  ...
  void try_process_next() {
    ...
    while (true) {
      std::span<const std::byte> _span(buffer_.data() + buffer_start_,
                                       buffer_end_ - buffer_start_);
      const std::size_t _msg_size = get_message_size(_span);
      if (_msg_size == 0 || _span.size() < _msg_size)
        break;  // LCOV_EXCL_LINE note: Ignored.

      std::span<const std::byte> _view(buffer_.data() + buffer_start_,
                                       _msg_size);
      buffer_start_ += _msg_size;

      // LCOV_EXCL_START
#ifndef NDEBUG
      fmt::println("{:%Y-%m-%d %H:%M:%S} SESSION READ ip={} port={} buffer={}",
                   std::chrono::system_clock::now(), ip_, port_,
                   span_to_hex(_view));
#endif
      // LCOV_EXCL_STOP
      ...
      try {
        switch (const auto _type = static_cast<request_types>(
                    std::to_integer<uint8_t>(_view[0]));
                _type) {
          case request_types::insert:
            write_buffer_[write_offset_] = state_->handle_insert(_view);
            _batch[_batch_size++] =
                boost::asio::buffer(write_buffer_.data() + write_offset_, 1);
            write_offset_ += 1;
            break;
          ...
          case request_types::get:
            state_->handle_query(request_query::from_buffer(_view),
                                 _type == request_types::query, _batch,
                                 _batch_size, write_buffer_.data(),
                                 write_offset_);
            break;
          case request_types::update:
            write_buffer_[write_offset_] =
                state_->handle_update(request_update::from_buffer(_view));
            _batch[_batch_size++] =
                boost::asio::buffer(write_buffer_.data() + write_offset_, 1);
            write_offset_ += 1;
            break;
          case request_types::purge:
            write_buffer_[write_offset_] =
                state_->handle_purge(request_purge::from_buffer(_view));
            _batch[_batch_size++] =
                boost::asio::buffer(write_buffer_.data() + write_offset_, 1);
            write_offset_ += 1;
            break;
  ...
  void do_write(const std::array<boost::asio::const_buffer, 64>& batch,
    ...
    // LCOV_EXCL_START
#ifndef NDEBUG
    fmt::println("{:%Y-%m-%d %H:%M:%S} SESSION WRITE ip={} port={} buffer={}",
                 std::chrono::system_clock::now(), ip_, port_,
    ...
    boost::asio::async_write(
        socket_, batch,
        boost::asio::bind_allocator(handler_allocator<int>(handler_memory_),
                                    [_self](const boost::system::error_code& ec,
                                            const std::size_t length) {
                                      _self->on_write(ec, length);
                                    }));
  }

  static std::size_t get_message_sized(const std::span<const std::byte> buffer,
                                       const std::size_t header_size,
                                       const std::size_t extra = 0) {
  ...
  static std::size_t get_message_size(const std::span<const std::byte> buffer) {
    ...
    switch (const auto _type = static_cast<request_types>(
                std::to_integer<uint8_t>(_buffer[0]));
            _type) {
      case request_types::insert: {
        auto* _h =
            reinterpret_cast<const request_insert_header*>(_buffer);  // NOSONAR
        return get_message_sized(buffer, request_insert_header_size,
                                 _h->key_size_);
      }
      case request_types::query: {
        auto* _h =
            reinterpret_cast<const request_query_header*>(_buffer);  // NOSONAR
        return get_message_sized(buffer, request_query_header_size,
                                 _h->key_size_);
      }
      case request_types::update: {
        auto* _h =
            reinterpret_cast<const request_update_header*>(_buffer);  // NOSONAR
        return get_message_sized(buffer, request_update_header_size,
                                 _h->key_size_);
      }
      case request_types::purge: {
        auto* _h =
            reinterpret_cast<const request_purge_header*>(_buffer);  // NOSONAR
        return get_message_sized(buffer, request_purge_header_size,
                                 _h->key_size_);
      }
      case request_types::set: {
        auto* _h =
            reinterpret_cast<const request_set_header*>(_buffer);  // NOSONAR
        return get_message_sized(buffer, request_set_header_size,
                                 _h->key_size_ + _h->value_size_);
      }
      case request_types::get: {
        auto* _h =
            reinterpret_cast<const request_get_header*>(_buffer);  // NOSONAR
        return get_message_sized(buffer, request_get_header_size,
                                 _h->key_size_);
      }
        // LCOV_EXCL_START
      default:
  ...
  void do_read() {
    ...
    socket_.async_read_some(
        boost::asio::buffer(buffer_.data() + buffer_end_,
                            max_length_ - buffer_end_),
        boost::asio::bind_allocator(
            handler_allocator<int>(handler_memory_),
            [_self](const boost::system::error_code& ec,
                    const std::size_t length) { _self->on_read(ec, length); }));
  }
