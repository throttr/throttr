...
class state : public std::enable_shared_from_this<state> {
 public:
  ...
  explicit state(boost::asio::io_context& ioc)
      : expiration_timer_(ioc), strand_(ioc.get_executor()) {}
   ...
   * @return
   */
  std::uint8_t handle_entry(const std::string_view& key,
    ...
    entry _scoped_entry{type,
                        value_owned{reinterpret_cast<const char*>(value.data()),
                                    value.size()},  // NOSONAR
                        ttl_type, _expires_at};
    ...
    if (_inserted) {
      if (const auto& _entry = _index.begin()->entry_;
          _expires_at <= _entry.expires_at_) {
    ...
    fmt::println(
        ...
        "RESPONSE ok={}",
        std::chrono::system_clock::now(), as_insert ? "INSERT" : "SET", key,
        span_to_hex(value), to_string(ttl_type), ttl, _inserted);
#endif
    // LCOV_EXCL_STOP
  ...
  std::uint8_t handle_insert(const std::span<const std::byte> view) {
    ...
    const std::span value(view.begin() + 1,
                          view.begin() + 1 + sizeof(value_type));
    return handle_entry(_request.key_, value, _request.header_->ttl_type_,
                        _request.header_->ttl_, entry_types::counter, true);
  }
  ...
  std::uint8_t handle_set(const std::span<const std::byte> view) {
    ...
    return handle_entry(_request.key_, _request.value_,
                        _request.header_->ttl_type_, _request.header_->ttl_,
                        entry_types::raw, false);
  }
   ...
   * @return uint8_t
   */
  void handle_query(const request_query& request,
                    bool as_query,
...
#ifndef NDEBUG
      fmt::println("{:%Y-%m-%d %H:%M:%S} REQUEST {} key={} RESPONSE ok=false",
                   std::chrono::system_clock::now(), as_query ? "QUERY" : "GET",
                   _key.key_);
#endif
      return;
    }
    ...
    if (as_query) {
      ...
      batch[batch_size++] =
          boost::asio::buffer(&_entry.ttl_type_, sizeof(_entry.ttl_type_));
      std::memcpy(write_buffer + write_offset, &_ttl, sizeof(_ttl));
      batch[batch_size++] =
          boost::asio::buffer(write_buffer + write_offset, sizeof(_ttl));
      ...
      batch[batch_size++] =
          boost::asio::buffer(write_buffer + write_offset, sizeof(_size));
    ...
    if (as_query) {
      ...
      fmt::println(
          ...
          "ttl_type={} ttl={}",
          std::chrono::system_clock::now(), _key.key_, *_quota,
          to_string(_entry.ttl_type_), _ttl);
    } else {
      fmt::println(
          "{:%Y-%m-%d %H:%M:%S} REQUEST GET key={} RESPONSE ok=true value={} "
          "ttl_type={} ttl={}",
          std::chrono::system_clock::now(), _key.key_,
          span_to_hex(std::span(
              reinterpret_cast<const std::byte*>(_view.pointer_), _view.size_)),
          to_string(_entry.ttl_type_), _ttl);
    }
#endif
  }
  ...
  std::uint8_t handle_update(const request_update& request) {
    ...
    storage_.modify(_it, [&](entry_wrapper& object) {
      switch (request.header_->attribute_) {
        case quota:
          if (object.entry_.type_ ==
              entry_types::counter) {  // LCOV_EXCL_LINE Note: Partially tested
            _modified = apply_quota_change(object.entry_, request);
          }
          break;
        case ttl:
          _modified =
              apply_ttl_change(object.entry_, request, _now, object.key_);
          break;
      }
    });
    ...
    fmt::println(
        ...
        to_string(request.header_->change_), request.header_->value_,
        _modified);
#endif
    // LCOV_EXCL_STOP
   ...
   * @return bool
   */
  static bool apply_quota_change(const entry& entry,
    ...
    switch (request.header_->change_) {
      ...
      case decrease:
        if (*_quota >=
            request.header_
                ->value_) {  // LCOV_EXCL_LINE note: Partially covered.
          *_quota -= request.header_->value_;
        } else {
          return false;
        }
  ...
  bool apply_ttl_change(entry& entry,
    ...
    if (scheduled_key_.size() == key.size() &&
        std::equal(scheduled_key_.begin(), scheduled_key_.end(),
                   key.begin())) {  // LCOV_EXCL_LINE Note: Partially tested
      boost::asio::post(strand_, [_self = shared_from_this(),
                                  _expires_at = entry.expires_at_] {
  ...
  std::uint8_t handle_purge(const request_purge& request) {
    ...
    // LCOV_EXCL_START
#ifndef NDEBUG
    fmt::println("{:%Y-%m-%d %H:%M:%S} REQUEST PURGE key={} RESPONSE ok={}",
                 std::chrono::system_clock::now(), _key.key_, _erased);
#endif
    // LCOV_EXCL_STOP
   ...
   * Expired entries
   */
  std::deque<std::pair<entry_wrapper, std::chrono::steady_clock::time_point>>
      expired_entries_;
  ...
  void expiration_timer() {
    ...
    for (const auto& _key : _to_expire) {
      if (auto _it = _valid_index.find(std::make_tuple(_key, false));
          _it != _valid_index.end()) {
        _valid_index.modify(_it, [](entry_wrapper& entry) {
#ifndef NDEBUG
          fmt::println(
              "{:%Y-%m-%d %H:%M:%S} SCHEDULER KEY EXPIRED key={}",
              std::chrono::system_clock::now(),
              std::string_view(reinterpret_cast<const char*>(entry.key_.data()),
      ...
      if (auto _it = _valid_index.find(std::make_tuple(_key, true));
          _it != _valid_index.end()) {
        if (auto _projected = storage_.project<tag_by_expiration>(_it);
            _projected != _timer_index.end()) {
#ifndef NDEBUG
          fmt::println(
              "{:%Y-%m-%d %H:%M:%S} SCHEDULER KEY ERASED key={}",
              std::chrono::system_clock::now(),
              std::string_view(reinterpret_cast<const char*>(_it->key_.data()),
...
#ifndef NDEBUG
    fmt::println("{:%Y-%m-%d %H:%M:%S} SCHEDULER GARBAGE COLLECTION STARTED",
                 std::chrono::system_clock::now());
#endif
...
#ifndef NDEBUG
    fmt::println("{:%Y-%m-%d %H:%M:%S} SCHEDULER GARBAGE COLLECTION FINISHED",
                 std::chrono::system_clock::now());
#endif
