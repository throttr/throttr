...
namespace throttr {
    bool create_service::use(const std::shared_ptr<state> &state, const std::string_view &key,
        const std::vector<std::byte> &value, const ttl_types ttl_type, uint64_t ttl, const entry_types type,
      ...
      if (_inserted)
      {
        ...
        for (const auto &_item : _index)
        {
          ...
          if (!_item.expired_)
          {
            ...
            {
              boost::asio::post(
                state->strand_, [_state = state->shared_from_this(), _expires_at] { _state->schedule_expiration(_expires_at); });
            }
            break;
          }
        }
